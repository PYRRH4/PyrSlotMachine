...
public class Machine extends DataElement {
	...
	protected final MachineBoard getBoard() {
		return PyrSlotMachine.inst().getData().getMachines();
	}
	...
	protected final void mysqlPull(ResultSet set) throws SQLException {
		...
		Map<String, String> cases = PyrCore.UNPRETTY_GSON.fromJson(set.getString("cases"), new HashMap<String, String>().getClass());
	...
	protected final Query getMySQLPushQuery() {
		...
		return new Query("REPLACE INTO `" + getBoard().getMySQLTable() + "`(`id`,`type`,`cases`,`button`) VALUES(?,?,?,?);", id, type, PyrCore.UNPRETTY_GSON.toJson(copy), 
